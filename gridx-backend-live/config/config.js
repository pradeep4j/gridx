...
// config.response = (rescode, message, data, res, extraData) => {
//     data = (JSON.stringify(data) === '{}') ? [] : ((Array.isArray(data) == false) ? [data] : data);
//     const status = rescode == 200 ? true : false;
//     res.status(200).json({ data: data, message: message, status: status, ...extraData });
// }
...
config.getUserBinary = async (value) => {
    ...
    const binaryreport = new BinaryReport(
        {
            userId: user?._id?.toString(),
            amount: b_comm,
            left_business: left_pv,
            right_business: right_pv,
            matching: min_pv,
            status: user.activ_member

        });
    binaryreport.save().then(() => {
    }).catch((error) => {
        config.response(500, 'Internal Server Error!', {}, res);
    });
    const incomHistory = new IncomHistory(
...
config.dailyBonusCrone = async (amount, uid, transin, transstatus) => {

    // const user = await Users.findOne({ _id: ObjectId(uid) });
    var activemember = 0
    let userdaily = await Users.findOne({ _id: ObjectId(uid), activ_member: '1' });
    amounttype = '$';
    var daily_bonusroiWallet;
    if (userdaily) {
        if (transstatus == 1) {
            ...
            ///use 3xcondition 
            roiWallet = userdaily.roiWallet + (daily_bonusroiWallet * liveRate.data.result.message);
            wallet = userdaily.wallet + (daily_bonusroiWallet * liveRate.data.result.message);
            //+ (daily_bonusroiWallet * liveRate.data.result.message);

            internalGDXWallet = userdaily.internalGDXWallet + daily_bonusroiWallet;
            let check3XFun = await config.check3XFun(uid);
            let check3XFunfinal = await config.check3XFunFinal(uid, check3XFun);

            if (check3XFunfinal[1] == false) {
                const filter = { _id: userdaily._id };
                const update = { activ_member: 0 };
                ...
                wallet = userdaily.wallet + 0;
                roiWallet = userdaily.roiWallet + 0;
                internalGDXWallet = userdaily.internalGDXWallet + 0;
                userdaily.activ_member = 0;
            }
            else {
                if (check3XFun[0].count * 3 < wallet) {
                    console.log('sdfs')
                    ...
                    const incomHistory = new IncomHistory(
                        {
                            userId: userdaily?._id?.toString(),
                            amount: wallet - walletdata,
                            type: 'roi',
                            category: 'credit',
                            status: 0,
                            description: 'Binary Reward 3x complete: $ ' + (wallet - walletdata),
                        });
                    incomHistory.save().then(() => {
                    ...
                    directamount = walletdata - userdaily.wallet;
                    wallet = walletdata;
                    console.log(directamount);
                    daily_bonusroiWallet = directamount / liveRate.data.result.message;
                    roiWallet = userdaily.roiWallet + directamount;
                    internalGDXWallet = userdaily.internalGDXWallet + (directamount / liveRate.data.result.message);
                }
                else {
                    roiWallet = userdaily.roiWallet + (daily_bonusroiWallet * liveRate.data.result.message);
                    wallet = userdaily.wallet + (daily_bonusroiWallet * liveRate.data.result.message);
                ...
                userdaily.activ_member = 1;
            }
            const filter = { _id: userdaily._id.toString() };
            const update = { wallet: wallet.toFixed(2), roiWallet: roiWallet.toFixed(2), internalGDXWallet: internalGDXWallet.toFixed(2) };
            const doc = await Users.findOneAndUpdate(filter, update, {
                returnOriginal: false
            }).exec((err, res) => {
            ...
            daily_bonusroiWallet = amount * 0.3 / 100;
            roiWallet = userdaily.roiWallet + daily_bonusroiWallet;
            wallet = userdaily.wallet + daily_bonusroiWallet;
            internalWallet = userdaily.internalWallet + daily_bonusroiWallet;
            let check3XFun = await config.check3XFun(uid);
            let check3XFunfinal = await config.check3XFunFinal(uid, check3XFun);
            if (check3XFunfinal[1] == false) {
                const filter = { _id: userdaily._id };
                ...
                wallet = userdaily.wallet + 0;
                directbonus = userdaily.directWallet + 0;
                internalWallet = userdaily.internalWallet + 0;
                userdaily.activ_member = 0;
            }
            else {
                if (check3XFun[0].count * 3 < wallet) {
                    ...
                    const incomHistory = new IncomHistory(
                        {
                            ...
                            category: 'credit',
                            status: 0,
                            description: 'Binary Reward 3x complete: GDX ' + (wallet - walletdata).toFixed(2),
                        });
                    incomHistory.save().then(() => {
                    ...
                    directamount = walletdata - userdaily.wallet;
                    wallet = walletdata;
                    daily_bonusroiWallet = directamount;
                    roiWallet = userdaily.roiWallet + directamount;
                    internalWallet = userdaily.internalWallet + directamount;
                }
                userdaily.activ_member = 1;
            }
            const filter = { _id: userdaily._id.toString() };
            const update = { wallet: wallet, roiWallet: roiWallet, internalWallet: internalWallet };
            const doc = await Users.findOneAndUpdate(filter, update, {
                returnOriginal: false
            }).exec((err, res) => {
    ...
    const dailybonus = new DailyBonus(
        {
            userId: uid,
            transId: transin,
            amount: (amount * 0.3 / 100).toFixed(2),
            usdamount: ((amount * 0.3 / 100) / liveRate.data.result.message).toFixed(2),
    ...
    const incomHistory = new IncomHistory(
        {
            userId: uid,
            amount: daily_bonusroiWallet.toFixed(2),
            type: 'roi',
            category: 'credit',
            status: activemember,
            description: 'Roi Comm : ' + (amounttype + daily_bonusroiWallet).toFixed(2),
        });
...
config.verifyOtp = async (user, type, code, res, value, add_min = 10) => {
    ...
    if (createotptime > new Date()) {
        if (userdata.otp !== code) {
            config.response(201, 'Invalid OTP!', {}, res);
        }
        else {
            var resMessage = '';
            const filter = { _id: userdata._id };
            var update = { otp_time: null, otp: null, updated_at: new Date() }
            if (type == 'mobile') {
                resMessage = 'Mobile Has been Verified Successfully!';
                update = { ...update, sv: 1, mobile: value };
            } else {
                resMessage = 'Email Has been Verified Successfully!';
        ...
        config.response(201, 'OTP Expired Please Resend', {}, res);
    }
}

config.sendOtpMethod = () => ["email", "mobile"];

config.getOTPExpireTime = () => {
    return parseInt(process.env.OTP_EXPIRE_TIME);
}
